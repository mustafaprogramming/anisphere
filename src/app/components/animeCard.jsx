...
const AnimeCard = ({anime}) => {
 ...
 return (
 <article key={id} className="lg:w-40 md:w-36 sm:w-32 w-28 relative">
       <div className="relative">
         <img src={image || ''} alt="anime cover" className="lg:h-60 md:h-56 sm:h-48 h-44 w-full"/>   
         ...
         onMouseOut={hideLink}
         >
          {showLink && <Link href={`/AnimeList/${id}`} className="absolute w-full h-full backdrop-blur-md">
       ...
       <h5 className="text-white font-semibold ">{(title.substring(0,18))+(title.length>=18?'...':'')|| 'N/A'}</h5>
